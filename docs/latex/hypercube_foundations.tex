...
The runnable pairs monitor does not use ML.  A separate checked-in research
harness asks a narrower downstream question: after a residual, trade side, and
candidate event have been declared causally, can a fitted model improve the
accept-or-reject decision?  This preserves rolling \(z\) as the control policy
and prevents a model from receiving credit merely for inventing a different
economic trade.
...
\]
An empty infimum is treated as infinity.
The first touch gives class \(+1\) or \(-1\); no touch before \(H\) gives class
zero.  The regression target retains economic magnitude at the same exit:
\[
  r_t=g_t\!\left(\min(\tau_+,\tau_-,H)\right)-c.
\]
Thus triple barrier is naturally a three-class problem, but it is not
necessarily the best target.  Classification discards within-class overshoot
and the variable vertical-barrier return; direct regression retains both
\cite{lopezdeprado}.

The temporal protocol separates five roles:
\[
...
\]
A complete \(H\)-observation purge gap lies between every adjacent block.
Validation controls boosting iterations.  The calibration block supplies a
point-prediction intercept correction, marginal quantile corrections, or
sigmoid class-probability calibration.  A later selection block chooses the
minimum accepted model score.  The final block is not used by any of those
operations.  Positions cannot overlap within a pair.

LightGBM and CatBoost supply ordinary squared-error, Huber, quantile, and
multiclass lanes \cite{lightgbm,catboost}.  Huber loss is quadratic near zero
and linear in the tails, limiting an extreme target's influence
\cite{huber}.  Quantile regression estimates conditional distribution points
rather than a conditional mean \cite{quantileregression}.  For
\(q\in\{0.2,0.5,0.8\}\), the calibration correction is
\[
  \Delta_q=\operatorname{Quantile}_q(r-\widehat Q_q(x)),\qquad
  \widetilde Q_q(x)=\widehat Q_q(x)+\Delta_q.
\]
Outputs are sorted per observation to prevent crossing, and the frozen
lower-tail utility is
\[
  u_Q=\widetilde Q_{0.5}
      -0.25\bigl(\widetilde Q_{0.5}-\widetilde Q_{0.2}\bigr).
\]
A strict \(\widetilde Q_{0.2}>0\) rule accepts essentially no reference-test
events because stop outcomes occupy more than one fifth of the distribution.
The less absolute utility above permits uncertainty-aware ranking.

Class-weighted three-way models receive an independent one-versus-rest sigmoid
calibrator because a useful ranking score need not be a probability
\cite{sklearncalibration}.  Their economic score restores payoff magnitude:
\[
  u_C(x)=\sum_{k\in\{-1,0,+1\}}
  P^{\mathrm{cal}}(Y=k\mid x)\,\bar r_k,
\]
where \(\bar r_k\) is estimated only in the calibration block.  On the
reference seed, 20th, 50th, and 80th percentile test coverage is approximately
0.18--0.20, 0.47--0.49, and 0.79--0.80; profit-class expected calibration
error is about 0.03.  Calibration quality is necessary for probability-based
decisions, but it does not guarantee superior event selection.

The experiment contains a homogeneous Gaussian OU negative control and a
positive control with observable volatility and persistence regimes.  Stressed
positive-control states add unit-variance Student-\(t(4)\) idiosyncratic shocks;
the true regime is withheld from the features.  Each reported lane comprises
two expanding folds over 20 pairs and 3,300 observations, repeated for five
...
The negative control favors the simple policy on average.  In the constructed
observable stress, both Huber lanes beat it in all five seeds.  This validates
the conditional-filter hypothesis and the harness, not market profitability.
The next evidentiary step is a frozen protocol on survivorship-controlled
market data with hedge parameters estimated inside each fold, bid/ask
execution, borrow, asynchronous-leg risk, portfolio constraints, and clustered
or block-bootstrap uncertainty.  The current models remain optional Python
research artifacts; no ML dependency enters the Rust runtime.
